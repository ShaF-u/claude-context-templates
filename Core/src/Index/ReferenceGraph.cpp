...
bool ReferenceGraph::IsSourceFile(const std::string& path) {
...
Result<void> ReferenceGraph::Build(const std::string& root, const std::vector<std::string>& extra_ignore_patterns) {
    ...
    for (const auto& metadata : scan_result.Value()) {
        ...
        Cache<std::vector<ReferenceEdge>>::PutOptions options;
...
Result<void> ReferenceGraph::UpdateFile(const std::string& root, const std::string& path) {
    ...
    if (!file.is_open()) {
        return Result<void>::Fail(Error{
            ...
            .module = "Core.Index.ReferenceGraph",
        });
    }
    std::ostringstream buffer;
    buffer << file.rdbuf();
    ...
    std::lock_guard lock(mutex_);
    edges_.erase(
        std::remove_if(edges_.begin(), edges_.end(), [&](const ReferenceEdge& e) { return e.referencing_file == path; }),
        edges_.end());
    edges_.insert(edges_.end(), std::make_move_iterator(file_edges.begin()), std::make_move_iterator(file_edges.end()));
    return Result<void>::Ok();
}

Result<void> ReferenceGraph::RemoveFile(const std::string& path) {
    {
        std::lock_guard lock(mutex_);
        edges_.erase(std::remove_if(edges_.begin(), edges_.end(),
                                     [&](const ReferenceEdge& e) { return e.referencing_file == path; }),
                     edges_.end());
    }
...
std::vector<ReferenceEdge> ReferenceGraph::References(const std::string& type_name) const {
    std::lock_guard lock(mutex_);
...
std::vector<ReferenceEdge> ReferenceGraph::AllEdges() const {
    std::lock_guard lock(mutex_);
    return edges_;
}

std::size_t ReferenceGraph::Size() const {
    std::lock_guard lock(mutex_);
    return edges_.size();
}
